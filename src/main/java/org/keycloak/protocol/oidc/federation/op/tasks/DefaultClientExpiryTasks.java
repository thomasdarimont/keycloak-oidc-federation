...
public class DefaultClientExpiryTasks implements ClientExpiryTasks {

    private static final Logger logger = Logger.getLogger(DefaultClientExpiryTasks.class);

    private final KeycloakSession session;

    protected DefaultClientExpiryTasks(KeycloakSession session) {
    ...
    public void scheduleTask(String id, String realmId, long expiresAt) {
        ...
        long interval = expiresAt - (long) Time.currentTime();
        ...
        timer.schedule(new OIDCFedScheduledTaskRunner(session.getKeycloakSessionFactory(), new DeleteExpiredClientScheduledTaskRunner(id, realmId), taskName), interval, taskName);
