...
public class FedUtils {
    ...
    public static String getSelfSignedToken(String issuer) throws IOException {
        issuer = issuer.trim();
        if (!issuer.endsWith("/")) issuer += "/";
        return getContentFrom(new URL(issuer + WELL_KNOWN_SUBPATH));
    }

    public static String getSubordinateToken(String fedApiUrl, String issuer, String subject) throws IOException {
    ...
    public static JSONWebKeySet getKeySet(KeycloakSession session) {
        ...
        for (KeyWrapper k : session.keys().getKeysStream(session.getContext().getRealm()).collect(Collectors.toList())) {
